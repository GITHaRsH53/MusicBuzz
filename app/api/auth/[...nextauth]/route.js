...
export const authOptions = {
  providers: [
    SpotifyProvider({
      ...
      profile(profile) {
        return {
          id: profile.id,
          name: profile.display_name || (profile.email ? profile.email.split("@")[0] : "Spotify user"),
          email: profile.email ?? null,
          image: Array.isArray(profile.images) && profile.images[0]?.url ? profile.images[0].url : null,
        };
      },
    }),
  ],
  secret: process.env.NEXTAUTH_SECRET,
  trustHost: true, // fine on Vercel
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      // Always send the user back to the frontend origin
      const frontend = process.env.NEXTAUTH_URL || baseUrl; // set to your FRONTEND domain in Vercel env
      if (url.startsWith("http")) return url;
      return frontend;
    },
  },
