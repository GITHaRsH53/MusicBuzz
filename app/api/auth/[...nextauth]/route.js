...
export const authOptions = {
  providers: [
    SpotifyProvider({
      ...
      // Map Spotify profile to the shape NextAuth expects
      profile(profile) {
        return {
          id: profile.id,
          name:
  ...
  // Make cookies third-party friendly in prod
  cookies: isProd
    ? {
        sessionToken: {
  ...
  callbacks: {
    ...
    async jwt({ token, account, user }) {
      ...
      // Save user profile once (or refresh if missing)
      if (user) {
        token.user = {
    ...
    // Expose user + access token to the client
    async session({ session, token }) {
      session.accessToken = token.accessToken ?? null;
      session.user = token.user ?? null;        // <— important
      return session;
    },

    async redirect({ url, baseUrl }) {
      if (url.startsWith("http")) return url;
      return process.env.FRONTEND_ORIGIN || baseUrl;
    },
  },
};
